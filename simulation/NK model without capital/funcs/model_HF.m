function [fyn, fxn, fypn, fxpn] = model_HF(flexp,setp)
...
%Steady State
[ss,setp,flexp] = model_HF_ss(flexp,setp); % 
...
syms y c n w m lam pii cc xi
syms y_p c_p n_p w_p m_p lam_p pii_p cc_p xi_p

% %Declare X and Y vectors
XX  = [a t chi betv r cc]; % vector of state variables. Note al is a at t-1, and so on
XXP = [a_p t_p chi_p betv_p r_p cc_p]; % p signifies t+1 
...
% Model Equations 
f(1) = - w*xi + psii*(1-n)^(-sigmn);
f(end+1) = - m + bet*(xi_p/xi);
% f(end+1) = - m + bet*(xi_p/xi)*(betv_p/betv);
f(end+1) = - 1 + m*(1+r_p)/pii_p;
f(end+1) = -xi + cc*betv - bb*bet*betv_p* cc_p;
% f(end+1) = -xi + cc - bb*bet*(betv_p/betv)* cc_p;
f(end+1) = - cc_p + (c_p-bb*c)^(-sigmc);
