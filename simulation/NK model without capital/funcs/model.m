function [fyn, fxn, fypn, fxpn] = model(flexp,setp)
...
syms y c n w m lam pii 
syms y_p c_p n_p w_p m_p lam_p pii_p
...
YY  = [y c n w m lam pii]; % vector of control variables
YYP = [y_p c_p n_p w_p m_p lam_p pii_p]; % p signifies t+1 
...
% Model Equations 
f(1) = - w + psii*c^sigmc*(1-n)^(-sigmn);
f(end+1) = - m + bet*(c_p/c)^(-sigmc)*(betv_p/betv);
