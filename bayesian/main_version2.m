...
for i=1:num_tot
    ...
    if i>num_burn
        ...
        IRFi = zeros(n^2, irf_length+1, 0);
        kerneli = [];
        for ddelta1 = 0:0.01:1
            dtheta1 = ddelta1^(-1)-1;
            if dsigma(1,2)<0
                if dtheta1 < -dsigma(1,1)/dsigma(1,2) && dtheta1 >= -dsigma(1,2)/dsigma(2,2)
                     dQ = calcGPFARotation(dsigma, ddelta1);
                     dcomp_mat = makeCompMat(dB, n, p);
                     [dA_0, dA_plus] = calcStructuralVARParams(dB, dsigma, ddelta1);
                     dJ_plus =  calcJacobianStructuralwrtReduced(dB, dsigma, ddelta1, precision);
                     dJ_minus =  calcJacobianStructuralwrtReduced(dB, dsigma, ddelta1, -precision);
                     dJ = 0.5*(dJ_plus+dJ_minus);
                     ...
                     IRFi(:,:, end+1) = calcStructuralIRF(dcomp_mat, dsigma, p, irf_length, dQ);
                     kerneli = [kerneli, pos_kernel];
                end
            end  
        end
        ...
        [Vi, Ii] = sort(kerneli, 'descend');
        IRFs_gpfa(:,:, i-num_burn) = IRFi(:,:, Ii(1));
        kernels_gpfa(i-num_burn) = Vi(1);    
    end
end
